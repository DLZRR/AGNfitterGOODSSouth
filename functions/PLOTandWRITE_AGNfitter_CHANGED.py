...
class OUTPUT:
    ...
    def plot_manyrealizations_SED(self):    
        ...
        # Do not know if 0 is the best fit index OBVIOUSLY it is not the best fit, will probably need one of the others, or the submitted code.
        special_array = np.column_stack((all_nus, TOTALnuLnu[0]))
        np.savetxt('/home/dlzr/Documents/AGNfitter/OUTPUT/' + str(self.data.name) + '/SED_Template_' + str(self.data.name) + '.txt', special_array)
...
def SED_plotting_settings(x, ydata, modeldata):
    ...
    **Input:
    ...
    ax1.set_ylim(mediandata /90.,mediandata * 100.)
    ax1.set_xlim(min(modeldata), max(modeldata))

    ax2.set_xscale('log')
    ax2.set_yscale('log')
    ax2.set_ylim( mediandata /90., mediandata * 100.)
    ...
    x2 = (2.98e14/ x)[::-1] # Wavelength axis
